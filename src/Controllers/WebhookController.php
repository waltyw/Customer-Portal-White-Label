...
class WebhookController
{
    ...
    private function handleCheckoutComplete(object $session): void
    {
        ...
        if ($payment['invoice_id']) {
            ...
            if ($invoice && $user) {
                $html = "<p>Hello {$user['name']},</p><p>We've received your payment of <strong>£" . number_format($payment['amount'], 2) . "</strong> for invoice <strong>{$invoice['invoice_number']}</strong>. Thank you!</p><p><a href=\"{$_ENV['APP_URL']}/invoices/{$invoice['id']}\" style=\"background:#2563eb;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block;\">View Invoice</a></p>";
